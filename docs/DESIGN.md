...
**`--fs-pill-label` 10px — узаконенное исключение, НЕ поднимать** (решение клиента
2026-07-21, вопрос закрыт). Это микро-лейбл внутри пилла (верхняя строка ChannelPill
...
- **Footer** (дизайн в коде 2026-07-21, идея — «острова меню приземлились»): рифмуется
  с хедером тем же примитивом `.island` — пара бренд-островов (логотип + вордмарк с
  тэглайном) 1:1 как в меню; тихий violet-glow за брендом (идиома §6, opacity .14);
  грид `360px | 1fr` — бренд + 3 колонки ссылок (якоря те же, что в меню); ряд
  32px-дисков каналов (половина орбитального диска, несквадратные лого хранят
  пропорции); нижняя полоса: копирайт → Privacy/Terms/En → CTA-кнопка (системная,
  `flex-shrink: 0`, чтобы не сжималась). ≤946 — колонка, ≤760 — колонки 2×2 и bar в стек.
